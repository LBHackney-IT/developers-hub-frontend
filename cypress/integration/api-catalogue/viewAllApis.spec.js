...
describe("All APIs Pagination", () => {
  ...
  it("View the first page", () => {
    cy.visit("/api-catalogue");
    cy.wait("@getApiDefinitions");
    cy.get(".lbh-simple-pagination__link--previous").should(
      "have.class",
      "disabled"
    );
  });
  ...
  it("View the last page", () => {
    visitLastPageIfPossible();
    cy.get(".lbh-simple-pagination__link--next").should(
      "have.class",
      "disabled"
    );
  });
...
describe("Filter APIs", () => {
  ...
  it("View inactive APIs", () => {
    ...
    cy.wait("@getApis")
      .its("request.url")
      .should("include", "state=UNPUBLISHED");
  });
...
describe("Resetting Pagination", () => {
  ...
  const scenarios = [
    {
      name: "switching filters",
      function: () => {
        cy.get("#filterApis-2").check();
      },
    },
    {
      name: "changing sort by",
      function: () => {
        cy.get("select#SortBy").select("A-Z");
      },
    },
